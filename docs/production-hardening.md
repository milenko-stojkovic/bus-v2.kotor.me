...
- **`guest_lower_category_checkout_blocked`** — guest checkout blokiran prije plaćanja (niža kategorija od najnovije guest **`paid`** istorije iste tablice); `admin_alerts` + email.
- **`guest_paid_lower_category_alert`** — safety-net poslije plaćanja ako guest **`paid`** rezervacija ipak ima nižu kategoriju od istorije (`reservation_id`, `historical_reservation_id`, cijene).
