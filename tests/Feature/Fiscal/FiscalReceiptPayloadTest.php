...
class FiscalReceiptPayloadTest extends TestCase
{
    ...
    public function test_real_driver_continues_when_deposit_returns_already_initialized(): void
    {
        config([
            'services.fiscalization.driver' => 'real',
            'services.fiscal.api_url' => 'https://fiscal.test',
        ...
        SystemConfig::query()->updateOrInsert(
            ['name' => 'document_number'],
            ['value' => 43, 'updated_at' => now()]
        );

        Http::fake([
            'https://fiscal.test/api/efiscal/deposit' => Http::response([
                'IsSucccess' => false,
                'Error' => [
                    'ErrorCode' => '56',
            ...
            'https://fiscal.test/api/efiscal/fiscalReceipt' => Http::response([
                'IsSucccess' => true,
                'ResponseCode' => 'JIKR-DEPOSIT-56',
                'UIDRequest' => 'IKOF-DEPOSIT-56',
                'Url' => ['Value' => 'https://efitest.tax.gov.me/ic/#/verify?iic=IKOF-DEPOSIT-56'],
            ]),
        ]);

        $reservation = $this->makePaidReservation((string) Str::uuid());

        $result = app(FiscalizationService::class)->tryFiscalize($reservation);

        $this->assertSame('JIKR-DEPOSIT-56', $result['fiscal_jir'] ?? null);
